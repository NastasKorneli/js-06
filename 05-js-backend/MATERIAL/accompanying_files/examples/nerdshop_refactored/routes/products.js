...
router.post('/', [isAuthenticated, isAdmin], async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ msg: 'Product added', status: 201, data: savedProduct });
  } catch (error) {
    console.error(error.message);
    res.status(500).json({ msg: 'Product could not be added.' });
  }
});
...
router.put('/', [isAuthenticated, isAdmin], async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ msg: 'Product updated.', status: 200, data: updatedProduct });
  } catch (error) {
    console.error(error);
    res.status(500).json({ msg: 'Product could not be updated.', error });
  }
});
